...
def test_repeat_incident_replays_learned_fix_first_attempt(tmp_path: Path) -> None:
    with FakeOpenAIServer(expected_tool_parser="qwen3") as server:
        ...
        learned_entries = (
            state["learned_fixes"][provider_id]["wrong_endpoint_port"]
        )
        assert learned_entries
        assert learned_entries[0]["successful_fix"] == "update_endpoint_url"

        inject_failure(config_path, "wrong_endpoint_port")
        second = self_heal_config(config_path, provider_name="rules")
        assert second.healthy, second.reason
        assert second.attempts == 1
        assert second.repairs[-1].recipe_id == "update_endpoint_url"

        inject_failure(config_path, "wrong_endpoint_port")
        third = self_heal_config(config_path, provider_name="rules")
        assert third.healthy, third.reason
        assert third.attempts == 1, (
            f"learned-fix replay should heal in 1 attempt, got {third.attempts}"
        )
        assert third.repairs[-1].recipe_id == "update_endpoint_url"
        assert len(third.repairs) == 1, (
            f"learned-fix replay should produce exactly one repair, got {len(third.repairs)}"
        )
